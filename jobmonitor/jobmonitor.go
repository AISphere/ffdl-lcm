...
func NewJobMonitor(trainingID string, userID string, numLearners int, jobName string, useNativeDistribution bool, statsdClient *statsd.Statsd, logr *logger.LocLoggingEntry) (*JobMonitor, error) {
	...
	jmMetrics := initMetrics(statsdClient)
	...
	if err != nil {
		...
		if err := updateJobStatusOnError(trainingID, userID, client.ErrCodeK8SConnection, service.StatusMessages_INTERNAL_ERROR.String(), logr, jmMetrics); err != nil {
	...
	if connectivityErr != nil {
		shutdownTrainingOnETCDFailure(trainingID, userID, jobName, connectivityErr, logr, jmMetrics)
...
func updateJobStatusInTrainer(trainingID string, userID string, statusUpdate *client.TrainingStatusUpdate, logr *logger.LocLoggingEntry, jmMetrics *jobMonitorMetrics) error {
	...
	if err != nil {
		jmMetrics.FailedTrainerConnectivityCounter.Add(1)
...
func updateJobStatusOnError(trainingID string, userID string, errorCode string, statusMessage string, logr *logger.LocLoggingEntry, jmMetrics *jobMonitorMetrics) error {
	...
	return updateJobStatusInTrainer(trainingID, userID, &statusUpdate, logr, jmMetrics)
}
...
func (jm *JobMonitor) processUpdateJobStatus(currStatus string, logr *logger.LocLoggingEntry) bool {
	...
	error := updateJobStatusInTrainer(jm.TrainingID, jm.UserID, statusUpdate, logr, jm.metrics)
	if error != nil {
	...
	if status == grpc_trainer_v2.Status_COMPLETED || status == grpc_trainer_v2.Status_FAILED || status == grpc_trainer_v2.Status_HALTED {
		logr.Infof("(processUpdateJobStatus) overall status of the job was set up as %s and native distribution status was %t", currStatus, jm.UseNativeDistribution)
...
// jobmonitor metrics
func initMetrics(statsdClient *statsd.Statsd) *jobMonitorMetrics {
...
func shutdownTrainingOnETCDFailure(trainingID, userID, jobName string, err error, logr *logger.LocLoggingEntry, jmMetrics *jobMonitorMetrics) {

	logr.WithError(err).Error("failed to connect to etcd while monitoring training and shutting down the job")
	if err := updateJobStatusOnError(trainingID, userID, client.ErrCodeEtcdConnection, service.StatusMessages_INTERNAL_ERROR.String(), logr, jmMetrics); err != nil {
		logr.WithError(err).Errorf("Failed to write the status %s for training %s to trainer", grpc_trainer_v2.Status_FAILED, trainingID)
	}
	if err := KillDeployedJob(trainingID, userID, jobName, logr); err != nil {
		logr.WithError(err).Errorf("Failed to kill the deployed job %s", trainingID)
