...
func (instance *coordinator) Get(path string, log *logger.LocLoggingEntry, opts ...clientv3.OpOption) ([]EtcdKVGetResponse, error) {
	...
	res, nrerr := retry(numRetries, 5*time.Second, "ETCD_GET", log.WithFields(logrus.Fields{"method": "GET"}), func() (interface{}, error) {
		...
		return instance.handleError(attempt, err, log)
	})
...
func (instance *coordinator) Put(path string, value string, log *logger.LocLoggingEntry, opts ...clientv3.OpOption) (EtcdKVPutResponse, error) {
	...
	res, nrerr := retry(numRetries, 5*time.Second, "ETCD_PUT", log.WithFields(logrus.Fields{"method": "PUT"}), func() (interface{}, error) {
		ctx, cancel := context.WithTimeout(context.Background(), timeout)
		defer cancel()
...
func (instance *coordinator) PutIfKeyExists(path string, value string, log *logger.LocLoggingEntry, opts ...clientv3.OpOption) (bool, error) {
	res, nrerr := retry(numRetries, 5*time.Second, "ETCD_PUT_IF_EXISTS", log.WithFields(logrus.Fields{"method": "ETCD_PUT_IF_EXISTS"}), func() (interface{}, error) {
		ctx, cancel := context.WithTimeout(context.Background(), timeout)
		defer cancel()
	...
	response, ok := res.(*clientv3.TxnResponse)
	if nrerr != nil || !ok {
		log.WithError(nrerr).Errorf("Exception while performing a transaction to update key %s , with value %s and options %v.", path, value, opts)
		return false, nrerr
	}
	log.Debugf("Completed the PutIfKeyExists operation for key %s , value %s and opts %v with result %t", path, value, opts, response.Succeeded)
	return response.Succeeded, nrerr
...
func (instance *coordinator) PutIfKeyMissing(path string, value string, log *logger.LocLoggingEntry, opts ...clientv3.OpOption) (bool, error) {
	...
	log.Debugf("Completed the  PutIfKeyMissing operation for key %s , value %s and opts %v with result %t", path, value, opts, response.Succeeded)
	return response.Succeeded, nrerr
}
...
func (instance *coordinator) CompareAndSwap(path string, newValue string, expectedOldValue string, log *logger.LocLoggingEntry, opts ...clientv3.OpOption) (bool, error) {
	...
	response, ok := res.(*clientv3.TxnResponse)
	if nrerr != nil || !ok {
		log.WithError(nrerr).Errorf("Exception while performing a transaction to compare and swap key %s , with newvalue %s , existingvalue %s and options %v", path, newValue, expectedOldValue, opts)
		return false, nrerr
	}

	log.Debugf("Completed the  CompareAndSwap operation for key %s , newvalue %s , existingvalue %s and opts %v with result %t", path, newValue, expectedOldValue, opts, response.Succeeded)
	return response.Succeeded, nrerr
}
...
func (instance *coordinator) DeleteKeyIfExists(path string, log *logger.LocLoggingEntry, opts ...clientv3.OpOption) (bool, error) {
	...
	response, ok := res.(*clientv3.TxnResponse)
	if nrerr != nil || !ok {
		log.WithError(nrerr).Errorf("Exception while performing a transaction to delete key %s and options %v.", path, opts)
		return false, nrerr
	}
	log.Debugf("Completed the  DeleteKey operation for key %s , opts %v with result %t", path, opts, response.Succeeded)
	return response.Succeeded, nrerr
}
...
func (instance *coordinator) DeleteKeyWithOpts(path string, log *logger.LocLoggingEntry, opts ...clientv3.OpOption) error {
	...
	_, nrerr := retry(numRetries, 5*time.Second, "ETCD_GET", log, func() (interface{}, error) {
		ctx, cancel := context.WithTimeout(context.Background(), timeout)
		defer cancel()
...
func (instance *coordinator) GetLeaseDetails(leaseID clientv3.LeaseID, log *logger.LocLoggingEntry) (*clientv3.LeaseTimeToLiveResponse, error) {
	...
	res, nrerr := retry(numRetries, 5*time.Second, "ETCD_LEASE_DETAILS", log, func() (interface{}, error) {
		ctx, cancel := context.WithTimeout(context.Background(), timeout)
		defer cancel()
...
func (instance *coordinator) RevokeLease(leaseID clientv3.LeaseID, log *logger.LocLoggingEntry) error {
	...
	_, nrerr := retry(numRetries, 5*time.Second, "ETCD_REVOKE_LEASE", log, func() (interface{}, error) {
		ctx, cancel := context.WithTimeout(context.Background(), timeout)
		defer cancel()
...
func connect(config Config, log *logger.LocLoggingEntry) (*clientv3.Client, error) {
	...
	endpointIdx := 0
	response, nrerr := retry(numRetries, 5*time.Second, "ETCD_CONNECT", log,
		func() (interface{}, error) {
			//start with 1 endpoint since etcd go client is not able to handle both endpoints cleanly. follow the logic in handleError() function to switch to other endpoint in case of failure
			endpoint := []string{config.Endpoints[endpointIdx]}
		...
		func(attempt int, err error) bool {
			endpointIdx = attempt % len(config.Endpoints) //shuffle endpoints, switches between 0,1
			log.WithError(err).Warnf("attempt=%d , ctx is canceled with a deadline is exceeded,  shuffling endpoint and trying again with endpoint %v", attempt, config.Endpoints[endpointIdx])
			return true
		})
...
//conditions for which the error should be handled and retry should be executed
func (instance *coordinator) handleError(attempt int, err error, log *logger.LocLoggingEntry) bool {
	var retry bool
	if err != nil {
		switch err {
		case context.DeadlineExceeded,
			rpctypes.ErrGRPCAuthFailed,
			rpctypes.ErrGRPCMemberBadURLs,
			rpctypes.ErrGRPCInvalidAuthToken:
			endpointIndex := attempt % len(instance.config.Endpoints) //switches between 0,1
			log.WithError(err).Warnf("attempt=%d , ctx is canceled with a deadline is exceeded, shuffling endpoint and trying again with endpoint %v", attempt, instance.config.Endpoints[endpointIndex])
			instance.cli.SetEndpoints(instance.config.Endpoints[endpointIndex]) //shuffle endpoints
			retry = true
		default:
			log.WithError(err).Error("unknow error not handled by current logic")
		}
	}
...
func retry(attempts int, interval time.Duration, description string, log *logger.LocLoggingEntry, operation func() (interface{}, error), condition func(attempt int, err error) bool) (interface{}, error) {
	var err error
	var result interface{}
	for i := 0; ; i++ {
		result, err := operation()
		//if there was no error or condition to retry was false
		if err == nil || !condition(i, err) {
			return result, err
		}
		log.WithError(err).WithField("description", description).WithField("attempt", i).Warnf("Failed in attempt, %d / %d for %s. Will retry after sleeping for %d", i+1, attempts, description, interval)
		if i >= (attempts - 1) {
			break
		}
		time.Sleep(interval)
